...
def load_data():
    ...
    url = "D:/UC Berkeley/Climate Site/Climate-Site/Synapse_project_Emma/Climate_site/python_scripts/institutions.tsv"
...
def related_projects(technologies, number_technology , carbon_related , size):
    ...
    if number_technology in dic_key_initiatives:
        ...
        if dic2 != {}:
            return reference_text,key_initiative,location_entities,pd.DataFrame(dic2).T.sort_values(by="abstract comparison" , ascending = False)[:size]
        else:
...
def main_authors(technologies,number_technology , carbon_related , size , research_words):
    ...
    if dic_papers_co_authors != {}:
        ...
        return pd.DataFrame(dic_papers_co_authors, index = [ "Author's name(s)" , "Author's id(s)" ,  "Number of occurence within the 100 most related papers" , "Last Known Institution" , "Number of works" , "Number of citations" ,  "Number of related citations"]).T.style.hide_index()
